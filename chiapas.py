...
conn = psycopg2.connect(database="prueba", host="alpha.tamps.cinvestav.mx", user="postgres", password="example", port="5440")
# conn = psycopg2.connect(database="prueba", host="localhost", user="postgres", password="example", port="5440")
...
def getProduct():
	...
	print(data)
	with conn.cursor(cursor_factory=RealDictCursor) as cursor:
		cursor.execute("SELECT p.id, p.methodology_id, p.url, p.levels, p.extension, p.key, p.type, r.stars FROM methodology_instance as p, (SELECT avg(rating) as stars FROM ratings WHERE product_id = %s) as r WHERE p.key = %s", (data, data, ))
		res = cursor.fetchone()
		response = {
			"data": res,
		}
...
def setRatingValue():
	...
	with conn.cursor(cursor_factory=RealDictCursor) as cursor:
		...
		cursor.execute("SELECT p.id, p.methodology_id, p.url, p.levels, p.extension, p.key, p.type, r.stars FROM methodology_instance as p, (SELECT avg(rating) as stars FROM ratings WHERE product_id = %s and methodology_id = %s) as r WHERE p.key = %s", (product_id, methodology_id, product_id, ))
		res = cursor.fetchall()
		print("la res", "*"*60)
		print(res)
		response = {
...
def getRelevantRatings():
	with conn.cursor(cursor_factory=RealDictCursor) as cursor:
		...
		cursor.execute("SELECT p.id, p.methodology_id, p.url, p.levels, p.extension, p.key, p.type, r.stars, r.product_id FROM methodology_instance as p, (SELECT avg(rating) as stars, product_id FROM ratings GROUP BY product_id ORDER BY stars DESC LIMIT 15) as r WHERE p.key = r.product_id ORDER BY r.stars DESC")
		res = cursor.fetchall()
		response = {
			"data": res,
		}
