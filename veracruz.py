...
def setRatingValue():
	...
	with conn.cursor(cursor_factory=RealDictCursor) as cursor:
		...
		cursor.execute("SELECT p.id, p.methodology_id, p.url, p.levels, p.extension, p.key, p.alias, p.type, r.stars FROM methodology_instance as p, (SELECT avg(rating) as stars FROM ratings WHERE product_id = %s and methodology_id = %s) as r WHERE p.key = %s", (product_id, methodology_id, product_id, ))
		res = cursor.fetchall()
		print("la res", "*"*60)
		print(res)
		response = {
